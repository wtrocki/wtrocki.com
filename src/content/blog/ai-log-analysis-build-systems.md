...
However, within an ecosystem like Evergreen, which manages a variety of build workloads, teams can potentially face a stream of failing tasks coming from platform engineering or widespread system tests.
...
Do not begin by asking a model to "fix build". Use narrow and targeted tool calls and helper workflows that reduce navigation, deduplicate failures, and return only the signals that matter. 